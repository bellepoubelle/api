...
public class SensorEndpoint {
	...
	public Response findById(@PathParam("id") Long id) {
		TypedQuery<Sensor> findByIdQuery = em
				.createQuery(
						"SELECT DISTINCT s FROM Sensor s WHERE s.id = :entityId ORDER BY s.id",
						Sensor.class);
	...
	public List<Sensor> listAll(@QueryParam("start") Integer startPosition,
		...
		TypedQuery<Sensor> findAllQuery = em.createQuery(
				"SELECT DISTINCT s FROM Sensor s ORDER BY s.id", Sensor.class);
	...
	@Path("/{id:[0-9][0-9]*}")
	@Consumes("application/json")
	public Response update(@PathParam("id") Long id, Sensor entity) {
		if (entity == null) {
